...
def main():
    ...
    #samples_a = tf.placeholder(tf.int32)
    #samples_b = tf.placeholder(tf.int32)

    if args.fast_generation:
        next_sample = net.predict_proba_incremental(samples, args.gc_id)
    else:
        next_sample_a = net.predict_proba(samples, args.gc_id)
        next_sample_b = net.predict_proba(samples, args.gc_id)
    ...
    waveform_generated = []
    if args.wav_seed:
        guide = create_guide(args.wav_seed,
                           wavenet_params['sample_rate'],
                           quantization_channels)
        waveform_guide = sess.run(guide).tolist()
        waveform = sess.run(guide).tolist()
	print(len(waveform))
    ...
    while(window_left+net.receptive_field < 80000):#len(waveform)):
        if args.fast_generation:
            ...
            if len(waveform) > net.receptive_field:
                ...
                window = waveform
            outputs_a = [next_sample_a]
            outputs_b = [next_sample_b]
	for x in range(20):

        	# Run the WaveNet to predict the next sample.
                prediction_a = sess.run(outputs_a, feed_dict={samples: window})[0]
                prediction_b = sess.run(outputs_b, feed_dict={samples: window})[0]
    ...
    if args.wav_out_path:
        out = sess.run(decode, feed_dict={samples: waveform_generated})
        write_wav(out, wavenet_params['sample_rate'], args.wav_out_path)
