...
def transform_song(input_song):
	...
	for i in range(1):#while last_mark < leng - 500:
		...
		print(next_mark)
		print(last_mark)
		song[last_mark:next_mark].export(seed_song_path, format="wav")
		
		main(seed_song_path, new_song_path, 8000)
		new_clip = AudioSegment.from_file(new_song_path, format="wav")
		if output_song == None:
	...
	output_song = output_song.fade_out()
	output_song.export(output_song_path, format="wav")
	print('DONE')
