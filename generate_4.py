...
def create_seed(filename,
                ...
                window_size,
                silence_threshold=0): #SILENCE_THRESHOLD):
...
def main(seeded_song, out_path, length, full_song=False):
    args = get_arguments()
    to_set = [['wav_seed', seeded_song], ['wav_out_path', out_path], ['samples', 8000], ['checkpoint', 'logdir/train/2017-12-08T20-37-28/model.ckpt-97750'], ['gc_channels', 32], ['gc_cardinality', 22], ['gc_id', 8]]
    for x in to_set:
	setattr(args, x[0], x[1])
...
#For transform song
	...
	while last_mark < leng - 6000:#net.receptive_field: #500:
	    next_mark = min(leng, last_mark + 500)#net.receptive_field)
	    seeds.append(song[last_mark:next_mark])
	    last_mark += 250#net.receptive_field//2
        
    num_times = 1	
    if full_song == True:
	print('hey')
	print(len(seeds))
	num_times = len(seeds)#//5    
	...
	print(j)
	    ...
	    if full_song:
		seeds[j].export(seed_song_path, format="wav")
